...
function updateLabelCountCallback(label) {
    ...
    //countlab(label.threadsUnread);
    //the below code adds speech to unread count but will auto triggered every 15 mins
    //let utterance = new SpeechSynthesisUtterance(label.threadsUnread);
...
function getBrowserActionAuthTokenCallback(token) {
    if (chrome.runtime.lastError) {
        ...
        // chrome.tabs.create({ 'url': 'js/popup.html' }); 
    }
